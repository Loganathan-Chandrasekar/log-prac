...
app.get("/analyst", async (req, res) => {
  ...
  try {
    ...
    const total = await Analyst.countDocuments();

    const analystsWithId = analysts.map(analyst => ({
      ...analyst.toObject(),
      id: analyst._id  // Add this line
    }));

    res.json({
      analysts: analystsWithId,
      currentPage: page,
...
// app.get("/analyst/byManagerTask/:managerTask", limiter, async (req, res) => {
...
app.get("/analyst/byManagerTask/:managerTask",  async (req, res) => {
  ...
  try {
    const totalAnalysts = await Analyst.countDocuments({ managerTask });
    const analysts = await Analyst.find({ managerTask })
      .sort({ dateTask: -1 }) // Sort by dateTask in descending order
      .skip(skip)
      .limit(limit);
    ...
    const totalPages = Math.ceil(totalAnalysts / limit);
...
app.get("/analyst/byTeamLead/:teamLead",  async (req, res) => {
  ...
  try {
    const totalAnalysts = await Analyst.countDocuments({ teamLead });
...
// app.get("/analyst/byTeamLead/:teamLead", limiter, async (req, res) => {
//   const teamLead = req.params.teamLead;
//   try {
//     const analysts = await Analyst.find({ teamLead });
//     if (analysts.length === 0) {
//       return res.status(404).json({ message: "No analysts found for the specified team lead." });
//     }
//     res.json(analysts);
//   } catch (error) {
...
app.get("/fetch/taskwise", async (req, res) => {
  try {
    ...
    if (projectName) matchCondition.projectName = projectName;
    ...
    const result = await Analyst.aggregate([
      ...
      { $sort: { projectName: 1, team: 1, task: 1 } }
    ]);

    const billableTasks = result.filter(item => 
      !['Training-Non Billable', 'Idle -Non Billable'].includes(item.task)
    );

    const nonBillableTasks = result.filter(item => 
      ['Training-Non Billable', 'Idle -Non Billable'].includes(item.task)
    );

// With these lines:
const billableEmployeeCount = billableTasks.reduce((sum, item) => sum + item.employeeCount, 0);
const nonBillableEmployeeCount = nonBillableTasks.reduce((sum, item) => sum + item.employeeCount, 0);
    

    const trainingCount = nonBillableTasks
      .filter(item => item.task === 'Training-Non Billable')
      .reduce((sum, item) => sum + item.employeeCount, 0);

    const idleCount = nonBillableTasks
      .filter(item => item.task === 'Idle -Non Billable')
      .reduce((sum, item) => sum + item.employeeCount, 0);
...
app.get("/api/taskwise/summary", async (req, res) => {
  try {
    const { startDate, endDate, selectedProject, selectedTeam } = req.query;

     // Parse and format dates
     const parsedStartDate = moment(startDate, ['YYYY-MM-DD', 'DD-MM-YYYY'], true);
     const parsedEndDate = moment(endDate, ['YYYY-MM-DD', 'DD-MM-YYYY'], true);
     ...
     let matchCondition = {
      dateTask: { 
        $gte: parsedStartDate.startOf('day').toDate(), 
        $lte: parsedEndDate.endOf('day').toDate()
      }
    };

    if (selectedTeam) matchCondition.team = selectedTeam;
    if (selectedProject) matchCondition.projectName = selectedProject;
      ...
      { $unwind: "$sessionOne" },
      {
        $group: {
          _id: {
            ...
            taskType: {
              $cond: {
                if: { $in: ["$sessionOne.task", ['Training-Non Billable', 'Idle -Non Billable']] },
                then: "nonBillable",
                else: "billable"
              }
            }
    ...
    // Process the result
    let billable = result.find(r => r.type === "billable") || { employeeCount: 0, totalTaskCount: 0 };
    let nonBillable = result.find(r => r.type === "nonBillable") || { employeeCount: 0, totalTaskCount: 0 };

    const response = {
      billable: {
        employeeCount: billable.employeeCount,
        taskCount: billable.totalTaskCount
      },
      nonBillable: {
        employeeCount: nonBillable.employeeCount,
        taskCount: nonBillable.totalTaskCount
      },
      total: {
        employeeCount: billable.employeeCount + nonBillable.employeeCount,
        taskCount: billable.totalTaskCount + nonBillable.totalTaskCount
      }
    };
    ...
    console.error("Error fetching taskwise summary:", error);
    res.status(500).json({ error: "Internal Server Error" });
  }
});


app.post('/checkExistingEmployee', async (req, res) => {
  try {
    const { emp_id, email_id } = req.body;
    const existingEmployee = await Employee.findOne({
      $or: [{ emp_id: emp_id }, { email_id: email_id }]
    });
...
// app.get("/fetch/userdata/", limiter, async (req, res) => {
//   const empId = req.query.empId;
//   const page = parseInt(req.query.page) || 1;
...
//   try {
//     const totalAnalysts = await Analyst.countDocuments({ empId });
//     const analysts = await Analyst.find({ empId })
//       .sort({ dateTask: -1 }) // Sort by dateTask in descending order
//       .skip(skip)
//       .limit(limit);
...
//       return res
//         .status(404)
//         .json({ message: "No analysts found for the specified employee ID." });
//     }
...
app.post("/update/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const { batch, jobs: { cDate } } = req.body;

    const billing = await Billing.findById(id);

    if (!billing) {
      return res.status(404).json("Billing not found");
    }

    // Add new batch history entry
    billing.batchHistory.push({
      count: batch,
      date: new Date(cDate)
    });
